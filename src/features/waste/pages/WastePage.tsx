...
export function WastePage() {
  ...
  async function load() {
    ...
    setError(null);
    try {
      const data = await client.get<unknown>("/api/waste");
  ...
  async function handleCreate(value: unknown) {
    ...
    try {
      await client.post("/api/waste", value);
      toast.success("Residuo creado.");
  ...
  async function handleUpdate(value: unknown) {
    ...
    setIsSubmitting(true);
    try {
      await client.put(`/api/waste/${encodeURIComponent(String(id))}`, value);
      toast.success("Residuo actualizado.");
      setEditRow(null);
      await load();
    } catch (err: unknown) {
  ...
  async function handleDelete(row: Row) {
    ...
    setIsSubmitting(true);
    try {
      await client.delete(`/api/waste/${encodeURIComponent(String(id))}`);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
        <div>
          <h2 className="text-xl font-semibold">Residuos</h2>
          <p className="mt-1 text-sm text-slate-700 dark:text-white/70">CRUD real contra `GET/POST/PUT/DELETE /api/waste`.</p>
        </div>
      ...
      <Dialog isOpen={isCreateOpen} title="Nuevo residuo" onClose={() => setIsCreateOpen(false)}>
        <JsonEditor initialValue={{}} onSubmit={handleCreate} submitLabel="Crear" isSubmitting={isSubmitting} />
      </Dialog>

      <Dialog isOpen={Boolean(editRow)} title="Editar residuo" onClose={() => setEditRow(null)}>
...
function inferColumns(rows: Row[]) {
  ...
  const base = ["id", "name", "type", "status", "createdAt", "updatedAt"];
...
function extractId(row: Row): unknown {
  return row["id"] ?? row["wasteId"] ?? row["uuid"] ?? row["_id"];
}
